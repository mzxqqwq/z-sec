...
| 9 | **Kali 是单点依赖**（2026-08-15 实测：API 服务崩溃致 1 小时评测白跑） | 无自动拉起/降级链路 | 比赛日风险最高项 |

> 已修复的历史瓶颈（供参考）：v4-pro 空输出（json 模式）、csawctf 前缀截断、空格 flag、占位符误交、目录大小写/空格判题、僵局击杀烧预算——详见第七节训练闭环表。
...
- **评测流程**：L1 冒烟（easy）→ L2 全量 → 短板报告（postmortem.py）→ 针对性优化 → 重测
...
| 版本 | 题集 | 成绩 |
|---|---|---|
| v0.5 | CTFTiny L1 静态（7 道 easy） | **7/7 全解** |
| v0.6 | CTFTiny L2（8 道 easy+very_easy，含 pwn） | **8/8 全解** |
| v0.10 | CTFTiny L2b（13 道 moderate） | **11/13（85%）** |
| v0.7 | DASCTF 2025 真题（7 道） | **3/7** |

能力画像：easy 100% · moderate 85% · 实战难度 43%——诚实、可复现、有版本对应。

**训练闭环实证**（每轮版本迭代靠评测发现真问题）：
...
| v0.10 | Kali 单点故障白跑 1 小时 | 评测健康闸门 | 快速止损 |
